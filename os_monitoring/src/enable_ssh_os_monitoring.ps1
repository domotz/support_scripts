...
if (Test-Path -Path $currentDir\Set-WmiNamespaceSecurity.ps1 -PathType Leaf) {
    Read-Host -Prompt "
IMPORTANT-> Do you want to use a non administrative user to unlock your Windows machine?
IF you do, please press ENTER, otherwise hit CTRL+C to QUIT!"
    ...
    Write-Output "
-> Flushing access policies by restarting Winmgmnt
    "
    net stop Winmgmt
    net start Winmgmt

    Write-Output "
-> Added permissions to WMI for user [$username]

############################################################################################################### 
SSH Configuration for OS_monitoring is completed for Administrative Users!

PLEASE NOTE THAT: It might take up to 3 hours to see the unlock option in the Domotz Access Management section, 
depending on the size of your network.

Then you will be able to use $username account to unlock your device using SSH
###############################################################################################################

"
}
else {
    Write-Output "-> Flushing access policies by restarting Winmgmnt: 
    "
    net stop Winmgmt
    net start Winmgmt
